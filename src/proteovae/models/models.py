...
class BetaVAE(BaseVAE):
    ...
    Args:
        ...
        encoder (~base.Encoder): Instance of an encoder-like architecture.  Works with any :class:`~nn.Module` object  mapping input
        tensors to hyperparameters of the posterior over the embeddings. 

        decoder (~base.Decoder): Instance of a decoder-like architecture. Works with any :class:`~nn.Module` object mapping embeddings to
        reconstructions. 
    ...
    def loss_function(self, data):
        ...
        .. math::
            ...
            \mathcal{L}(\boldsymbol{\theta}, \boldsymbol{\phi}; x) &= \mathbb{E}_{z \sim q_{\boldsymbol{\phi}}(z|x)}\left[\log p_{\boldsymbol{\theta}}(x|z)\right] - 
                \beta D_{KL}(q_{\boldsymbol{\phi}}(z|x)||p_{\boldsymbol{\theta}}(z))
            \end{align*} 
        ...
        .. math :: 
            ...
            &- \beta\in \mathbb{R} \text{ coefficients}
            \end{align*}
        ...
        recon = nn.functional.binary_cross_entropy(decoded.view(*X.shape), X)
        recon*=(28*28)
...
class JointVAE(GuidedVAE):
    ...
    def forward(self, x):
        ...
        # sampling
        z_cont = mu + torch.exp(0.5*log_var) *(torch.zeros(mu.shape, device=self.device).normal_())
